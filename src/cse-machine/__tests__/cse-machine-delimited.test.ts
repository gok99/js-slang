...
async function expectResult(code: string, options: object) {
...
// dc1.js: basic reset
// ============================================================
test('dc1: reset(() => 1 + 2 * 3) === 7', async () => {
  const value = await expectResult(`reset(() => 1 + 2 * 3);`, optionEC)
  expect(value).toBe(7)
})
...
// ============================================================
test('dc2: 1 + reset(() => 2 * shift(k => k(3) + k(5))) === 17', async () => {
  const value = await expectResult(
...
// ============================================================
test('dc3: mutation and shift with variable capture', async () => {
  const value = await expectResult(
    `
    let a = 0;
    let b = 0;
...
// ============================================================
test('dc4: currying via delimited continuations', async () => {
  const value = await expectResult(
    `
...
// ============================================================
test('dc5: call/cc implemented via shift', async () => {
  const value = await expectResult(
    `
...
// ============================================================
test('dc6: list prepend with shift', async () => {
  const value = await expectResult(
    `
    let ks = null;
...
// ============================================================
test('dc7: prepend_index with shift', async () => {
  const value = await expectResult(
    `
...
// ============================================================
test('dc8: multi-shift with mutation', async () => {
  const value = await expectResult(
    `
    reset(() => {
...
// ============================================================
test('effect1: basic handler with perform', async () => {
  const value = await expectResult(
    `
    withHandle(
      { test: (k) => k(41) },
...
// ============================================================
test('effect2: stateful handler (get/set/ret)', async () => {
  const value = await expectResult(
    `
    withHandle(
      {
...
// ============================================================
test('effect3: decide handler (maximize)', async () => {
  const value = await expectResult(
    `
    function choose(x, y) {
...
// ============================================================
test('effect4: error handler (raise discards continuation)', async () => {
  const value = await expectResult(
    `
    function divide(x, y) {
        if (y === 0) {
  ...
  expect(value).toBe('division by zero')
})

// ============================================================
// effect5.js: pythagorean triples via backtracking
// ============================================================
test('effect5: pythagorean triples via backtracking', async () => {
  const value = await expectResult(
    `
...
// ============================================================
test('effect6: handler discards continuation, returns computed value', async () => {
  const value = await expectResult(
    `
    withHandle(
      { test: (k, a, b) => a / b },
...
// ============================================================
test('effect7: multi-shot handler continuation', async () => {
  const value = await expectResult(
    `
    1 + withHandle(
      { test: (k) => k(3) + k(5) },
...
// ============================================================
test('effect8: fibonacci via accumulate effect', async () => {
  const value = await expectResult(
    `
    withHandle(
      {
...
// ============================================================
test('basic reset returns value from thunk', async () => {
  const value = await expectResult(`reset(() => 42);`, optionEC)
  expect(value).toBe(42)
})

test('shift can discard continuation', async () => {
  const value = await expectResult(`reset(() => 1 + shift(k => 42));`, optionEC)
  expect(value).toBe(42)
})

test('shift can access values from outer scope', async () => {
  const value = await expectResult(
    `
    const x = 10;
    reset(() => x + shift(k => k(5)));
...
test('shift outside reset captures full control', async () => {
  const value = await expectResult(`shift(k => 42);`, optionEC)
  expect(value).toBe(42)
})

test('basic withHandle returns value from body', async () => {
  const value = await expectResult(`withHandle({}, () => 42);`, optionDelimCont)
  expect(value).toBe(42)
})

test('nested handlers with different operations', async () => {
  const value = await expectResult(
    `
    withHandle(
      { outer: (k, x) => k(x + 100) },
...
test('handler continuation can be called multiple times', async () => {
  const value = await expectResult(
    `
    withHandle(
...
// ============================================================
test('scheduler: cooperative multithreading', async () => {
  const value = await expectResult(
    `
    let q = null;
...
test('perform without handler throws error', () => {
  return expectParsedError(
    `perform("unknown", 5);`,
    optionDelimCont
  ).toEqual('Error: No handler found for operation: unknown')
})
